...
app.get('/api/linkedin/callback', async (req, res) => {
  ...
  try {
    ...
    if (!state) {
      throw new Error("Missing state parameter from LinkedIn callback");
    }
    ...
    try {
      ...
      console.error("Failed to parse state:", state, parseErr);
      throw new Error("Invalid state parameter format");
    }

    if (!userId || userId === '00000000-0000-0000-0000-000000000000') {
      throw new Error("Invalid userId in state parameter. User may not be authenticated.");
    }

    console.log(`LinkedIn callback: Processing for userId=${userId}`);
    ...
    // Upsert profile row in Supabase (create if missing)
    const updateResponse = await fetch(`${supabaseUrl}/rest/v1/profiles?on_conflict=user_id`, {
      method: 'POST',
      headers: {
        'apikey': supabaseKey,
    ...
    if (!updateResponse.ok) {
      ...
      throw new Error(`Database upsert failed (${updateResponse.status}): ${errText}`);
    }
